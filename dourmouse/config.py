...
class OllamaConfig:
    ...
    def model_for_agent(self, agent: str | None) -> str:
        ...
        (orchestrator only, local only) the persisted orchestrator-model
        setting, then the built-in fast-dispatch default (also local only --
        see ``is_cloud`` above), then the run's default model.

        Real, live-caught bug (2026-09-20, research_pipeline's own live
        verification hit this): the persisted setting is tagged only by
        backend NAME ("ollama"), never by locality, so a choice saved while
        running LOCAL Ollama (this machine's own persisted value was
        "qwen2.5:7b") was still trusted after ``OLLAMA_API_KEY`` later made
        this SAME config cloud -- sending a local-only model name to
        https://ollama.com/api/chat, a real, reproducible 404 on every
        non-escalated orchestrator turn. This is the exact mirror of the
        bug ``skip_persisted_orchestrator_choice`` already exists to
        prevent (a CLOUD-saved value leaking into a force_local config) --
        that fix only ever covered one direction. Rather than guess which
        locality an "ollama"-tagged value was actually saved under (the
        stored tag genuinely cannot say), this follows the same "an
        ambiguous tag is never auto-applied" rule webui.py's own untagged-
        model-id save path already uses: once this config is cloud, the
        persisted choice is skipped entirely in favor of the real,
        known-correct cloud default. A user's deliberately-chosen cloud
        model (saved while already on cloud) is a real, separate,
        not-yet-built follow-on -- it would need the save path to tag
        locality too, not just backend name.
        """
        key = (agent or "").strip().upper()
        if key and key in self.agent_models:
            return self.agent_models[key]
        if (
            key == "ORCHESTRATOR"
            and not self.skip_persisted_orchestrator_choice
