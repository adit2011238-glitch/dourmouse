...
class TestRunCommandGuard:
    ...
    def test_tool_permission_tiers(self):
        ...
        # v13.2: write_path was ungated on explicit user request — the git
        # safety net (auto_commit + undo_last_change) now makes an
        # unwanted overwrite recoverable, unlike when this was first
        # gated (v8.15, pre-Aider-port). delete_path stays gated: a
        # deletion has no auto-commit to revert.
