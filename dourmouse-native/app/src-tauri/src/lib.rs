...
/// Real HTTP call to the existing dourmouse.webui server. Returns the raw
/// JSON body as a string on success, or an honest "ERROR: ..." string on
/// any failure (connection refused, timeout, non-200) — never fabricates
/// a result, same Rule 2.1/2.2 discipline the Python codebase follows
/// everywhere else.
#[tauri::command]
async fn fetch_dourmouse_status(base_url: String) -> String {
    let url = format!("{}/api/hands_free/status", base_url.trim_end_matches('/'));
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![greet, fetch_dourmouse_status])
