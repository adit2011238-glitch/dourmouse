...
### 144 -- the OS shell foundation, the HOME and SECURITY screens, and the tools the other 16 screens are built with (redesign, slice 1)

Severity: feature. Status: DONE 2026-09-26 (slice 1 of the redesign; the other 16 screens follow, and `/` still serves the console until one deliberate swap commit).

- **The shell** (`ui/shell.html`, `ui/assets/os/`: `boot.js`, `core/`, `kit/`, `chrome/`, `shell.css`, about 3,800 lines of native ES modules, no build step, no inline script). Chrome: menu bar with live status, sidebar and dock (with the three screens that stay in the classic console as links), stage with traffic lights that act on the stage, a directive box, Control Centre and Notification Centre on real reads (alerts dismiss for real on the server), wallpaper picker, accent that persists locally and on the server, SPEC overlay on Alt+A. One shared `/api/events` stream with resync, one fetch wrapper whose errors carry the server's own message (a 200 with `ok:false` is an error too), one chat stream parser, per-screen threads, the approval card and its bookkeeping, a bounded list helper, a keymap that ignores typing in fields and keeps an Esc stack, and a router that shows "not built yet" for a missing screen without a 404 in the console. `window.__dmShell.stats()` reports live subscriptions, timers and listeners so a screen that leaks can be caught.
- **HOME** (the conversation with tool chips that expand to the real arguments and results, approval cards, STOP, queue, project scope, attention list) and **SECURITY** (posture of this Mac from `/api/security_dashboard`, live through `security_*` events, findings, a flowchart of how a finding is made with real counts, SCAN NOW, FULL REPORT, LOCK behind a confirm card, REMEDIATE that hands the finding to HOME where the approval gate applies). Backend in `dourmouse/os_api/home.py`: which screens exist, one tab's own conversation read back, NEW THREAD.
- **Reviewed by the main thread** after the builder finished: the escaping (`html.js` escapes all five characters, `md.js` escapes first and allows only http and https links), the approval card (prompt text as text, focus on the card and never on APPROVE), the fetch wrapper, router, screen context, chat, events, prefs and the two screens were read; the builder had already replaced three regexes that were quadratic on 200 KB (the markdown renderer took 42 s, the affirmation check hung over 120 s). Fixed in review: the conversation view was locked inside HOME, so it now lives in `kit/thread-view.js` for the five other thread screens (COMMS, RESEARCH, MEDIA, CODE, NEWS); `states.populated` treated any object with a `text` field as markup (now only a real `SafeHtml`); screens get `ctx.overlays` so BROWSER can hide the Electron BrowserView while a panel is open; decided approvals are forgotten past 300; a backend module that fails to import is now recorded and named by `GET /api/os/ping` and skipped, instead of taking the other screens' routes (and the first request) down with it, which matters with seven builders writing modules at once; a mypy error in a test.
- **Tools for the other builders:** `scripts/os_shell_check.py` starts an isolated server, drives real Chrome, and reports per screen the data state, console errors, failed requests and whether a revisit grew the subscription, timer or listener counts; `--stub-chat` exercises a thread screen with a scripted stream and no model. `~/Documents/DOURMOUSE/REDESIGN_SCREEN_BUILDER_GUIDE.md` is the distilled contract.

Tests: `test_os_shell_route.py`, `test_os_core.py` (61, node harnesses over the shipped modules: error mapping, events and resync, ring, keymap, escaping, regex time limits on 200 KB, registry, chat and approvals, prefs and contrast on all five accents, timers and unmount cleanup), `test_os_screen_contract.py` (generic: every screen folder is checked with no edit), `test_os_screen_home.py`, `test_os_screen_security.py`, `test_os_api_router.py`. Live, in real Chrome at 1440x900 against an isolated server: zero console errors across HOME, SECURITY, a not-built screen and back; the counters return to their first-visit values; both panels close on Esc; a forced 500 shows the server's own words and RETRY recovers; killing and restarting the server resyncs SECURITY with no reload; a real alert was listed and dismissed on the server; SCAN NOW ran a real read-only scan of this Mac. Evidence: `EVIDENCE/144_os_home.png`, `144_os_security.png` and the mockup beside them.

Honest limits: a reply from a real large cloud model was never seen through HOME (the isolated config had no cloud key, so streaming, tool chips, approvals and STOP were checked against a scripted stream, labelled as such on screen); the Electron window was not driven (only Chrome); the LOCK path was declined, not approved, because approving changes this Mac; `/api/security/action` has no server-side approval gate, so the page's confirm card is the only ask (same as the console); warn, medium and the AI stage stay amber whatever the accent, on purpose, where the mockup lets them follow it; the aurora and ember wallpaper gradients still carry an amber glow in `dourmouse-os.css`; the builder ran a broad `pkill -f dourmouse.webui` once, which could have closed the owner's running app if it was open (nothing of Dourmouse was running afterwards, and it cannot be proven whether it was before).
...
### 146 -- OS shell screens NEWS, ATLAS and WIKI (redesign, wave A)

Severity: feature. Status: DONE 2026-09-26. Built by one Sonnet builder from the screen guide; the two backend routes and the NEWS link and confirm flow were read by the main thread, the rest is covered by the contract test, node helper tests, route tests and the live check.

- **NEWS** shows the headlines the server's news watcher collected: one catch-up read (`/api/news`), then each new headline arrives as a live `news_item` event with no refetch (newest 60 kept in a bounded list), the watcher's real status and poll interval, and a conversation with the news agent underneath (the shared thread view). TO RESEARCH first shows a confirm card that says exactly what it will do, then `POST /api/os/news/research` writes one research record (the headline as the question, the link as its first source) through the store the `research_*` tools use. It calls no model and fetches nothing; the pipeline's own model stages are left for the owner to run from RESEARCH. The route accepts only a real http(s) URL (no spaces or control characters, at most 2,000 characters), a title of at most 300 characters and a plain channel name, and is idempotent per headline and link.
- **ATLAS** is the world monitor (never linked to the quant lab of the same name): one row per feed the server really registered with the count it returned, feeds that failed or lack a key shown with their own reason (Flights HTTP 429, Infrastructure HTTP 406 and a bad RSS were seen live), the server's pulse label and score with a note on what actually moves the score, and REFRESH through `POST /api/os/world/refresh`, throttled to one re-poll per 15 s and one at a time (a refusal is HTTP 200 with `ok:false` so the browser logs no 4xx).
- **WIKI** is read only: counts computed from the real entries, a filterable list capped at 200 rows, and OPEN showing the stored summary; SCAN is shown disabled with the reason (there is no scan route, and a scan would send file contents to a cloud model).
- The mockup's invented numbers (eight channels, 44 events, headlines, file names, "120 s") are gone. Added in review: `ctx.keys.pushEsc` (the builder had to listen on its own root because a screen could not reach the shell's Escape stack).

Tests: `test_os_api_news.py` (18), `test_os_api_world.py` (4), `test_os_screen_news.py` (9), `test_os_screen_atlas.py` (8), `test_os_screen_wiki.py` (9), the contract test on the three folders (21). Live, in real Chrome against an isolated server: no console errors, no leak on revisit, all six states seen on each screen, a headline pushed through the real watcher and event hub appeared with zero refetches and its `<b>` tags rendered as text, TO RESEARCH wrote a record that was read back from the store, keyboard and Esc worked. Evidence: `EVIDENCE/146_os_news.png`, `146_os_atlas.png`, `146_os_wiki.png` with the mockup beside each.

Honest limits: the live-headline test used a fake source behind the real watcher and hub (its poll interval reads as 100000 s in the screenshot); several error, stale and unavailable states were forced by intercepting requests, not by a failing server; ATLAS omits the console's map, globe, layer toggles, scrubber and brief because the approved mockup has only the Channels and Pulse cards; NEWS tags a headline by channel (news, disaster, conflict), not by publisher, because the feed has none; two headlines with the same first 300 characters share one research record; the route imports two private helpers from `research_pipeline_tools` and would break if they were renamed (a test pins the behaviour); a cold `GET /api/world/pulse` blocks for up to about 10 s and the loading state says so.

### 147 -- OS shell screens GOALS, TIMETABLE, ORCHESTRATION and OFFICE, and a real pause for goals (redesign, wave B)

Severity: feature (the goals change touches a stored table). Status: DONE 2026-09-26. One Sonnet builder from the screen guide; the `goals.py` change and the goals routes were read by the main thread.

- **GOALS pause was not real.** A naive PAUSE would write the status `PAUSED` over the status the goal had, and a task already running would then write its own status over the pause. `goals.py` now keeps the previous status in its own column (`paused_from`, added by an `ALTER TABLE` on open and tested against an old-shape database). A worker write while paused is held in that column and applied on RESUME, a terminal write (completed, failed, expired) applies at once and clears the pause, and CANCEL clears it. Pausing lets a task already running finish and then nothing further starts. `create_goal_with_steps` makes a goal from steps the owner wrote (each depends on the one before; with no steps the objective is the one task; there is no planner). Routes in `os_api/goals.py`: a single board read (progress counted from real task rows), pause, resume, create, with every id, length and count validated; the expected refusals are 404 and 409.
- **GOALS screen:** every goal with its tasks, the exact action a task is parked on before the owner approves it, the audit history; every change (pause, resume, cancel, create, task approve or decline) is the owner's click plus a card saying what will happen. **TIMETABLE:** the real routines with EDIT (changes only when; the parser's own reason on refusal) and DISABLE or ENABLE, a routine on a gated tool is marked as unable to run unattended, overdue routines show under honest catch-up wording instead of the mockup's false "never backfilled"; NEW ROUTINE hands the request to HOME because a routine needs the model to choose a gated tool. **ORCHESTRATION:** follows the fan-out events live with no polling, recent runs from the persisted office log, TRANSCRIPT of a branch by call id. **OFFICE:** the 43-agent roster as 6 floors from a fixed table in `os_api/office.py` (an agent the table does not name lands on an Unassigned floor and is counted; the rule is printed on screen), real per-agent status from `/api/activity` and events, a meeting room driven only by events, FOLLOW and TRANSCRIPTS.
- Added in review: `kit/confirm-card.js` (`confirmHere`), the page-level confirm that SECURITY, GOALS and TIMETABLE each had copied; later screens use it.

Tests: `test_goals_pause.py` (18, including the pause-versus-running-task race and the migration), `test_os_api_goals.py` (13), `test_os_api_office.py` (4), the four screen test files (22), the contract test on the four folders (28), and the existing 106 goal tests still pass. Live, real Chrome at 1440x900, isolated server: about 70 checks passed (pause and resume restore the exact prior status, a declined card changes nothing, a bad schedule shows the parser's reason and changes nothing, real fan-out events drew branches with no refetch, a real tool event flipped an agent to working, Tab reached every control, revisits leaked nothing). Evidence: `EVIDENCE/147_os_{goals,timetable,orchestration,office}.png` with the mockup beside each.

Honest limits: the goal runtime and the scheduler runner were off in the live checks (nothing ran real tools or spent credits), so the pause race is proven in unit tests, not with a real model task; a goal whose task was running when the process died while paused is not recovered (existing runtime behaviour); a rollback to older code would see status PAUSED and treat it as stopped; the fan-out events carry no per-branch tool count, queued state or call id (a `webui.py` tracker change would add them), so none is drawn; the schedule store keeps no run history, so the mockup's "missed" row is an overdue tag computed from the browser's clock; there is no always-on "live" agent status in any source, so none is drawn; the goal runtime raises no events, so GOALS polls every 5 s; the event-stream-dropped banner was not exercised live; NEW ROUTINE's hand-off to HOME was only seen as a declined confirm card, never sent, to avoid spending credits.

### 148 -- swap preparation: the shell can be the default page, Electron can open it, and the sign-in check moves into the shell

Severity: feature. Status: DONE 2026-09-26. Nothing switches yet: the defaults stay as they were until the one swap commit.

- `DOURMOUSE_DEFAULT_SHELL=os` makes `/` serve the OS shell (with its CSP); unset or anything else still serves the console. `/console` and `/console.html` always serve the console, so the classic screens with no slot in the new shell (VISION, GLOBE, DESIGN3D) stay one address away.
- `DOURMOUSE_ELECTRON_START_PATH` (in `electron/main.js`) sets where the Electron main window opens, for both the first load and the macOS re-activate load. It accepts only a plain same-origin path (letters, digits and a few punctuation marks, no `//`, at most 120 characters) and falls back to `/workspace`, which is still the default.
- The console's startup sign-in check was a separate light-themed overlay script that would flash on the dark shell and is only injected on the console. The shell now has its own (`chrome/startup-check.js`): it reads `/api/connections` and `/api/auth/status`, and only if the Claude CLI, the Codex CLI or the Google account is reported not signed in does it open a dialog with the exact command to paste (click to copy) or the Google sign-in link, with "Continue anyway" and Esc to close. A failed read shows nothing, as before, which means "no problem reported", not "all signed in". The loading overlay was dropped on purpose (the shell paints at once and the check never blocks it).

Tests: `TestRootRouteAndElectronStartPath` (3) and `TestStartupCheck` (1) plus the import-safety case for the new module. Live in real Chrome with the two endpoints stubbed: three items with the right commands and the Google link, Esc closes it and the Esc stack returns to zero, nothing appears when everything is signed in, zero console errors.

Honest limits: the Electron start path was tested as source, not by launching Electron; the swap commit still has to flip the defaults, bump the service worker cache, add the shell files to its precache list, and re-point the root-route tests that pin the console's startup-script tag.

### 149 -- OS shell screens COMMS and AGENTSMITH (redesign, wave C)

Severity: feature (writes to the owner's mailbox and approves code that will run). Status: DONE 2026-09-26. Built by one Sonnet builder; the builder was cut off before its own live verification and report, so the main thread read both backends line by line and ran the live check.

- **COMMS.** The old `/api/gmail/search` parses only `(uid N)` rows, so it returned no rows at all for a signed-in Google user (whose rows end in `(id N)`): the console's mail list could only ever work with an App Password. A new `gmail_inbox_rows` reads the real mailbox (sender, subject, Gmail's own timestamp, unread and starred state, snippet) through the OAuth token. Routes in `os_api/comms.py`: inbox (a short cache that says how old it is; `fresh=1` forces a live read), one message, and archive, trash, flag and unflag. Each write is the owner's click behind a confirm card that names the message and what will happen. Trash is Gmail's Trash (recoverable for 30 days); nothing can delete permanently; sending mail is not here at all and stays on the model's approval gate. The new `gmail_flag` only adds or removes the STARRED label and is not registered as an agent tool. Every id is validated, every size is bounded, and a Google or IMAP failure is returned in its own words. With no mail account configured the screen says so with the exact setup line, never an empty inbox.
- **AGENTSMITH.** The review gate for a tool Dourmouse drafted for itself. The board lists every draft and whether an approved tool is actually live in the running server (approved is not live until a restart, and the screen shows both facts). The draft view shows everything the owner must read before approving: the exact module text that would be written and run, its sha256, the draft's own test, the parameters schema, and for an approved draft whether the file on disk still matches the hash recorded at approval. `POST /api/os/agentsmith/approve` requires the sha256 of the module text as it stands now, so the owner approves the bytes they read: a draft edited after it was opened is refused with 409, and a draft too large to read on screen cannot be approved there. The route calls the same `self_extensions.approve` as the existing route and adds no way for a model to approve.
- Review note: the git-free read paths and every write path were read; nothing widens what a web page or another local process can already do (the request guard sits in front of the router).

Tests: `test_os_api_comms.py` (49), `test_os_api_agentsmith.py` (21), `test_os_screen_comms.py` (18), `test_os_screen_agentsmith.py` (18). Live, real Chrome at 1440x900 against an isolated server: both screens mount with no console or page errors and no failed requests, a revisit leaves the subscription, timer and listener counts unchanged, and COMMS' thread ran with a scripted stream and its approval card. Evidence: `EVIDENCE/149_os_comms.png` (the honest "Not available" state: no mail account in the isolated server) and `149_os_agentsmith.png`.

Honest limits: COMMS was never run against a real Gmail account in this session (there is no signed-in Google user in the isolated server), so the row list, the four row actions and the confirm cards are covered by route and node tests, not clicked through with real mail; AGENTSMITH's approve flow was not clicked through in a browser either; the screenshots are not compared side by side with the mockup.

### 150 -- OS shell screens PROJECTS and SETTINGS (redesign, wave D)

Severity: feature (changes the owner's settings file). Status: DONE 2026-09-26. Same circumstances as #149.

- **PROJECTS.** A read that fails loudly (the old route answers 200 with a hidden error field), a plan route that shows the exact folder a new project would create before it is created, and a create route with no path argument on purpose (the folder is always `~/Documents/Dourmouse Projects/<name>`; a custom path is refused), bounded name and description, filesystem errors turned into honest refusals. Stopping tracking never deletes anything. Opening a project changes the tab's scope so the next chat and approval carry the project's own thread.
- **SETTINGS.** One summary read where every section reports its own failure without blanking the others: the native shell choice (Auto, Electron or pywebview, applied at next launch, refused if Electron is not installed), the models (with a plain note when the active backend is local, because the rule is large cloud models only), whether each API key is set and where from (never the value), the bind host read from the running socket, whether the access token gate is on, and the background switches with the security ones first. Writes: five named toggles (auto-approve sits behind a stronger confirm that says what it removes and is never a one-click tile), one background switch, the shell choice, and a reset that is scoped to the background switches only and refuses any other scope (`reset_features` in `settings_registry.py` removes just those saved lines, keeps every other line and the file at 0600, and says which switches are still overridden by the process environment). Every answer is a real HTTP status (the old routes report a failed write as 200 with `ok:false`).
- Review note: `_remove_user_settings` rewrites the user config file (which holds API keys) from what it read; it keeps every other key and the 0600 mode, and a test covers it, but it is the one place in this batch that rewrites a credentials file.

Tests: `test_os_api_projects.py` (16), `test_os_api_settings.py` (22), `test_os_screen_projects.py` (10), `test_os_screen_settings.py` (13). Live: both screens mount clean, no leaks on revisit; SETTINGS shows the real state of the isolated server. Evidence: `EVIDENCE/150_os_settings.png`, `150_os_projects.png`.

Honest limits: no toggle, reset or project create was clicked through in a browser (creating a project writes to the real Documents folder, so it was deliberately not done against the live path; the tests use a temporary Documents root); the reduced-motion switch is shown as following the OS setting because no server or client switch exists for it; the SETTINGS screenshot was taken against the demo server's default backend (a local Ollama), which the screen correctly flags as against the model rule.

### 151 -- OS shell screens CODE and RESEARCH (redesign, wave E)

Severity: feature. Status: DONE 2026-09-26. Same circumstances as #149.

- **CODE** is read-only. Routes in `os_api/code.py` list the projects it may read (this checkout and the projects on the owner's bookshelf that are the top of a git work tree, by id: the caller never sends a path), and give the working-tree status with per-file counts, one file's diff (working tree or one commit, with a context setting), the log, and the files of one commit. It runs `git` with no writes (no checkout, reset, stash, add or commit), without taking the index lock, with pathspecs taken literally after `--`, file paths that are relative and free of `..`, and an untracked file shown only if it is a regular file under 100 KB. The main thread added `--no-textconv` to every diff and `-c core.fsmonitor=false` to every call, so a repository's own configuration cannot make a diff or status run a program. The UI-4 leftovers are here: a diff widget and a terminal widget inside the screen folder.
- **RESEARCH.** Read-only over what the pipeline stored: the loop with each of the 15 stages "built" only when the tool that does the work is registered in this server's registry (a stage with no tool, such as source validation and statistics, is shown as not built with the reason), the backward edge built only when its tool is registered and it imports, real counts from the graph, the questions with their stage and open tasks, one question in full (claims with passage and source, contradictions), the neighbourhood of a question or hypothesis as nodes and typed edges (three hops, at most 40 nodes), and an export as markdown or JSON that the browser turns into a download (the server writes no file). A source card widget is in the screen folder.
- **Tests:** `test_os_api_code.py` (19), `test_os_api_research.py` (13), `test_os_screen_code.py` (13), `test_os_screen_research.py` (10). Live: both screens mount clean, no leaks on revisit; CODE showed this repository's real uncommitted diff, RESEARCH showed the real loop with 13 of 15 stages having a tool. Evidence: `EVIDENCE/151_os_code.png`, `151_os_research.png`.

Honest limits: RESEARCH was only seen empty (nothing researched in the isolated server), so the question list, claims, graph and export are covered by route and node tests, not by a browser run with real research data; CODE has no approve or revert (there is no backend for a staged diff that respects the human approval gate, and the screen says the model's file edits still ask first, reading the real gate list); a git repository that is not the top of a work tree is refused, not guessed.

### 152 -- OS shell screens MEDIA and VOICE (redesign, wave F)

Severity: feature (reads files on this Mac, uses the microphone path). Status: DONE 2026-09-26. Same circumstances as #149.

- **MEDIA.** The bytes are still served by the existing range-request routes; the shell never builds a URL from text the owner typed. `os_api/media.py` vouches for a path only if it is absolute after `~` and symlink resolution, is a real file with an audio, video or image extension (including the formats the converter handles), and lives inside a short fixed list of folders (the workspace, uploads, Movies, Music, Downloads, Desktop, Documents); anything else is refused with the reason. It lists recent files (re-checked on every read) or one named folder (never a path, not recursive, at most 100 files), and on open records the file in a small recent list and asks ffmpeg for the codecs and duration on a worker thread with an 8 s deadline, so an ffmpeg that never answers cannot hang the request. Whether the converter is installed is checked without running it (running it can hang on a machine whose first security scan has not finished). The screen carries a thread, as the console's does.
- **VOICE.** The command parser and the capability report already existed and the screen calls them as they are (the console's VOICE screen never used the parser). New: one honest read of the wake-word state (the environment gate and whether the two audio packages are installed, decided without importing them, because importing the audio library can block on the microphone permission prompt for the whole server), whether the hands-free loop is actually running, the parser's own command list so the help text cannot drift from the grammar, and a transcribe route that takes base64 audio inside JSON (the shell's one client sends JSON only), capped at 8 MB, refusing honestly when voice is off, and performing no action itself.
- **Tests:** `test_os_api_media.py` (10), `test_os_api_voice.py` (6), `test_os_screen_media.py` (7), `test_os_screen_voice.py` (7). Live: both screens mount clean, no leaks on revisit; MEDIA showed its empty state and VOICE its state. Evidence: `EVIDENCE/152_os_media.png`, `152_os_voice.png`.

Honest limits: playback, seeking, the media clock, conversion and subtitles were not exercised: they need the second-Electron check (the in-app browser tool cannot drive the media clock), which is still to do; the microphone path was not used; the transcribe route was tested with fixtures, not real speech.
